...
# Runtime imports of the *Config dataclasses used as config field annotations below.
# Required because `from __future__ import annotations` (top of file) turns these into
# string annotations, and OmegaConf.structured() -> get_type_hints() (called by
# cs.store) eagerly evaluates them on Python >=3.12 + omegaconf 2.3.x. Keeping them
# only under TYPE_CHECKING raises NameError at registration. These modules do not
# import src.trainers, so there is no circular-import risk.
from src.core.direction import DirectionsConfig
from src.criterions.base_criterion import CriterionBaseConfig
from src.loggings.progress_bar import ProgressBarConfig
from src.preprocessors.base_preprocessor import BasePreprocessorConfig
