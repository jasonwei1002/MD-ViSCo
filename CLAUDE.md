...
⚠️ **`trainer.learning_rate` and `trainer.scheduler_patience` are checkpoint-path LABELS, not live training knobs.** The optimizer is a Hydra partial mounted at `trainer.optimizer` (`/optimizer@optimizer: adam`, `lr` default **1e-3**); `create_optimizer` (`trainer.py:1307`) builds it from that partial and uses `self.learning_rate` **only as a fallback when no optimizer config exists** — which never happens for the shipped trainers. So `trainer.learning_rate=2.5e-4` changes the path string but the optimizer still runs at **1e-3**. Likewise the scheduler partial at `trainer.scheduler` (`reduce_lr_on_plateau`, `patience` default **10**) ignores `trainer.scheduler_patience`. The **live** knobs are `trainer.optimizer.lr` and `trainer.scheduler.patience`; `trainer.early_stopping.patience` *is* live (real instantiated component). To actually lower the LR you must set **`trainer.optimizer.lr`** (keep `trainer.learning_rate` equal for an honest path label), and set `trainer.scheduler.patience < early_stopping.patience` or the plateau scheduler can never decay before ES stops. (Run `rat-2` set only `trainer.learning_rate=2.5e-4` → trained at 1e-3, diverged at epoch 2, LR never decayed. `pretrain.sh`/`finetune.sh` now set `trainer.optimizer.lr=2.5e-4` + `trainer.scheduler.patience=3`.)
...
- `trainer.overwrite_checkpoint=true` is **required for any multi-epoch run**. The best-model checkpoint is a single rolling file (`epoch=None` → `_best` suffix, e.g. `PPG+ECG2BP_checkpoint_S_42_best.pt`; the periodic slot uses `_epoch_N`), re-saved every time val improves (`scalar_regression_trainer.py:642-667`). With the resolved default `overwrite_checkpoint=false`, `CheckpointManager.build_path(key="save", overwrite=False)` raises `ValueError: Checkpoint exists ... overwrite is False` on the **second** best-improvement (or first, if a stale ckpt from a prior run exists) — so training crashes mid-run. `config.yaml` marks `overwrite_checkpoint: ???`, i.e. it is a user decision; the schema default fills it to `false`, which only survives a compose check, not real training. Deleting the stale file alone is **not** a fix (next improvement re-crashes).
...
- ⚠️ **Paper-vs-config mismatch (affects what you reproduce).** This `ppg2bp_ecg2bp` (both PPG+ECG required, aggregated) matches the paper's **multi-input ablation** (Appendix E / Table XIII: `PPG+ECG→ABP`), **not** its headline refinement. The paper's main BP result (§III.D, the AAMI/BHS numbers) trains **two single-source branches** `PPG→ABP` and `ECG→ABP` jointly (MAE summed over `i∈{ECG,PPG}`) and **infers from one modality**. The naming convention is inconsistent: the sibling `ppg2abp_ecg2abp.yaml` ships **two** single-source directions (`[PPG]→ABP`, `[ECG]→ABP` → `direction_mode=multi`), but `ppg2bp_ecg2bp.yaml` ships **one** multi-source direction. ⚠️ Note the two-single-source-direction approach (`direction_mode=multi`) is **broken with BPModel** (mixed-batch mis-routing — see "BP refinement recipe" below). The working reproduction of the headline is `SOURCE=joint` (`refinement_trainer_mdvisco_pulsedb_joint`): one multi-source direction `[PPG,ECG]→BP` for correct per-vital routing **plus** a per-vital summed-MAE loss — **not** a two-entry directions combo.
...
The `pretrain.sh`/`finetune.sh` scripts default to **`SOURCE=joint`** — the paper's **headline** BP refinement (§III.D + Table II): both PPG+ECG present per sample, **each single-source branch supervised independently**, per-branch MAE **summed** over `i∈{ECG,PPG}` (`L_MAE = λ Σ_i (|sbp_i−SBP|+|dbp_i−DBP|)`), **single-modality inference**. Trainer: `refinement_trainer_mdvisco_pulsedb_joint` (added). `SOURCE` also accepts `ppg`/`ecg` (true single-encoder models), `multi` (Appendix-E multi-input ablation: `[PPG,ECG]→BP` with predictions **averaged**, both signals required at inference), and `dual` (see warning).

⚠️ **`_dual` is BROKEN with BPModel — do not use it; use `joint`.** `refinement_trainer_mdvisco_pulsedb_dual` sets `direction_mode=multi`, so the collate builds **mixed per-sample-direction batches** (`_add_multi_direction_metadata` randomly assigns one direction per sample). But `BPModel.extract_input` keys the **whole batch** off `directions[0]` (`mdvisco.py:3900-3912`) — it labels every sample's gathered channel with sample-0's modality, so ECG samples get routed through the PPG encoder (and vice-versa) → cross-modality contamination. `BPModelConfig.supports_multi_directional=False` flags this, but the trainer never consults it (`_get_model_supports_multi` at `trainer.py:735` is **dead code** — defined, never called; the collate is built straight from `direction_mode`). The `joint` path avoids this entirely by using **one** multi-source direction `[PPG,ECG]→BP` (`direction_mode=single`), which routes per-vital correctly (every sample runs both encoders, each on its own modality), and a per-vital **summed-MAE** loss instead of `_dual`'s mixed batches.

Why not just `multi`? The multi-input direction `[PPG,ECG]→BP` routes correctly but supervises MAE on BPModel's **averaged** prediction (`torch.stack(...).mean`), which is the Appendix-E ablation — the paper reports averaged-then-single-modality inference **degrades**. `joint` reuses the same correct routing but swaps the loss to per-vital summed MAE, so each branch is independently trained and single-modality inference matches training.

Added configs (joint, paper-headline):
- `src/conf/trainer/refinement_trainer_mdvisco_pulsedb_joint.yaml` — clone of `_pulsedb` but `override /directions@directions: ppg2bp_ecg2bp` (one multi-source direction, `direction_mode` stays `single`) **and** `override /criterion@criterion: regression_multi_wcl_pervital`; bakes in `criterion.regression_loss_term.scale_factor=0.001` (paper `λ_MAE`).
- `src/criterions/multi_branch_regression_loss.py::MultiBranchL1Loss` (+ `multi_branch_l1_loss.yaml`) — reads per-vital `<vital>_sbp`/`<vital>_dbp` from the model's `per_vital_bp` output, computes per-branch L1 vs the same SBP/DBP target, and **sums** over vitals; falls back to MAE on the aggregated prediction when no per-vital dict is present. `scale_factor` default 1.0 (joint overrides to 0.001).
- `regression_multi_wcl_pervital.yaml` — composes the stock `RegressionMultiWeightedContrastiveLoss` with `regression_loss_term=multi_branch_l1_loss`. The stock combined criterion's `forward` now **forwards its `**kwargs`** (incl. `per_vital_bp`) to the regression term (`regression_multi_weighted_contrastive_loss.py:217`), and `L1Loss`/`MSELoss.forward` accept+ignore `**kwargs` — so `MultiBranchL1Loss` sees the per-vital predictions with **no special subclass** needed. (The earlier `RegressionMultiWCLPerVital` subclass was removed by `/simplify` in favour of this one-line parent generalization.)
- `src/conf/directions/ppg2bp_ecg2bp_dual.yaml` + `refinement_trainer_mdvisco_pulsedb_dual.yaml` — the original two-single-source-direction attempt; **kept only for reference** (broken per above).
- `src/conf/train_dataset/train_pulsedb_refinement_pretrain_bp.yaml` — refinement **pretrain** split: `file_name: Train_Subset.h5`, **80/20**, `test_ratio: 0.0` (the "pretraining" scenario). Same preprocessing/targets as `train_pulsedb_refinement_bp`. Used by all variants.

**Loss hyperparameters now match the paper (Table X).** WCL (`multi_wcl_loss.yaml`): the 4 waveform terms were aligned from bare defaults (`τ_s=1, T_s=0, τ_w=1, λ=1e-3`) to the paper's `temperature_weight=τ_s=4, threshold=T_s=0.0235, temperature_embeddings=τ_w=4, scale_factor=λ1=0.01` (`T_s=0.0235` with `τ_s=4` ⇒ pairs within ~15 mmHg retained, matching the paper's "≤15 mmHg"). The text (age/gender) terms already matched. `λ_MAE=0.001` is baked into the joint trainer. Caveat: the paper's single per-modality waveform term uses a **combined** `S*=½(exp(SBP)+exp(DBP))`; this impl keeps the upstream **split** into separate SBP/DBP terms (each at `λ1`, mirroring the two patient terms each at `λ2`), so waveform-WCL total weight is ~2× the paper's — note before claiming bit-exactness. The non-joint paths (`ppg`/`ecg`/`multi`/`dual`) still use `l1_loss` (MAE weight 1.0, **not** `λ_MAE`-scaled); only `joint` is fully loss-consistent with the paper.
...
**Two scripts** (run from repo root): `pretrain.sh` (Step 1, discovers the produced checkpoint and **prints** its path plus a ready-to-run finetune command — no file handoff), and `finetune.sh <ckpt>` (Step 2, warm-starts from the **required** `$1` checkpoint path; `COLD_START=1` trains from scratch). There is no auto-discovery in `finetune.sh` and no `.last_pretrain_ckpt` file — copy the path `pretrain.sh` prints. Both scripts pass `trainer.save_checkpoint_frequency=null` (periodic saving **off** — only the best slot is kept). They share a `SOURCE` selector (default `joint`) — **`SOURCE` must match between `pretrain.sh` and `finetune.sh`** (warm-start strict-loads weights). Checkpoint filename embeds the direction key and ends in `_best`: `joint`/`multi` ⇒ `PPG+ECG2BP_checkpoint_S_42_best.pt`, `ppg`/`ecg` ⇒ `PPG2BP_`/`ECG2BP_` prefix, `dual` ⇒ no prefix. `pretrain.sh`'s discovery glob `*checkpoint_S_*_best.pt` matches all. ⚠️ `joint` and `multi` share the **same** prefix **and** subfolder (same model/hyperparams/direction) → do **not** run both into the same weights dir.
...
- Added the two `*_pulsedb` configs documented above, plus: the **paper-headline `joint`** path (`refinement_trainer_mdvisco_pulsedb_joint.yaml`, `criterions/multi_branch_regression_loss.py` + `multi_branch_l1_loss.yaml`, `regression_multi_wcl_pervital.yaml` — which reuses the stock combined criterion after making its `forward` + `L1Loss`/`MSELoss.forward` accept/forward `**kwargs`); the **single-source** trainers/models (`refinement_trainer_mdvisco_pulsedb_{ppg,ecg}.yaml`, `mdvisco_refinement_pulsedb_{ppg,ecg}.yaml`); the now-deprecated `_dual` configs (`ppg2bp_ecg2bp_dual.yaml`, `refinement_trainer_mdvisco_pulsedb_dual.yaml`, kept for reference); `train_pulsedb_refinement_pretrain_bp.yaml`; the WCL hyperparameter alignment in `multi_wcl_loss.yaml`; and the `pretrain.sh`/`finetune.sh`/`train.sh` scripts (`SOURCE` selector, default `joint`).
...
- `src/trainers/trainer.py` — added **memory-optimization** trainer fields (dataclass + `__init__` + `_run_epoch`/`run_training` branches), all defaulting OFF so upstream behavior is unchanged:
  - `use_amp: bool` + `amp_dtype: str` ("bfloat16"|"float16") — wraps the **whole `_step_core`** (forward + processor metrics + loss) of every stage in `torch.autocast(device_type="cuda", ...)`; CUDA-only (`amp_enabled = use_amp and device.type=="cuda"`, no-op on CPU/Mac). bf16 needs no loss scaling; fp16 routes backward through a `torch.amp.GradScaler` (built in `__init__`, `is_enabled()` only for fp16+CUDA, so bf16/off take the plain `loss.backward()` path). Near-lossless; big activation-memory saving + faster. ⚠️ Because **metrics run inside autocast**, model-output (prediction) tensors are bf16 when they reach the metric path — which broke a NumPy conversion until the `metrics_utils.py` fix below.
  - `use_gradient_checkpointing: bool` — new `_enable_gradient_checkpointing()` walks `self.model.modules()` and calls HF `gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": False})` (False required for DDP `find_unused_parameters=True`) on every submodule that supports it, **before** DDP wrap. Mathematically lossless; ~20-30% slower. ⚠️ **`PatchTSMixerModel` does not support it** (`supports_gradient_checkpointing=False`, verified at runtime in **both** transformers 4.39.3 **and 5.9.0** — `gradient_checkpointing_enable()` raises `ValueError`, caught + warned, skipped); only `DistilBertModel` does (`True`). So for refinement, checkpointing covers only the text branch (1 submodule enabled); the PatchTSMixer **waveform** branch relies on bf16 AMP. The `pretrain.sh`/`finetune.sh` scripts pass `trainer.use_amp=true trainer.amp_dtype=bfloat16 trainer.use_gradient_checkpointing=true`. **Env note**: the `mdvisco` env was later upgraded to **transformers 5.9.0 + numpy 2.4.4 + scikit-learn 1.8.0**, which **resolved** the earlier numpy-1.x C-extension ABI mismatch — full `--cfg job` compose now passes locally, and the PatchTSMixer/DistilBERT encoder forwards + `_enable_gradient_checkpointing` behavior are runtime-verified on 5.9.0. bf16 autocast itself is still GPU-only (server-verify). Note 5.x is a major jump from MD-ViSCo's 4.39.3 baseline; the two HF touchpoints (both read `.last_hidden_state`) are verified compatible, but full end-to-end forward+WCL is confirmed only by a real run.
- `src/processors/metrics_utils.py::_maybe_denormalize_tensor` — upcast to **float32 before NumPy** (`tensor.detach().cpu().float().numpy()`). Under bf16 AMP the prediction tensor is bfloat16 when it reaches the metric/denormalize path (autocast wraps the whole `_step_core`), and NumPy has no bf16 dtype → `TypeError: Got unsupported ScalarType BFloat16`, which crashed the **first** train epoch. `.float()` is lossless for bf16→f32 and a no-op for f32; only the single NumPy boundary needed it (the rest of the metric math stays in torch, which handles bf16). CPU-unit-verified with a bf16 input (no CUDA needed).
- `src/processors/metrics_utils.py::_calculate_bhs` — return a **per-batch BHS percentage** (`100 * count / N`) instead of a raw **count**. The trainer's `_summarize_processor_metrics._flatten` `float()`s each leaf and the metric container then **averages across batches** (same per-batch-mean→cross-batch-mean as MAE/ME), so a count aggregated to "mean hits per batch" → the logged `*/bp_bhs_*` keys came out **>100** (e.g. SBP ≤5mmHg ≈ 95, DBP ≤15mmHg ≈ 403 in run `swan-1`), impossible for a percentage and wildly inconsistent with the MAEs. `_calculate_bhs` is called only from `compute_bp_metrics` (no count consumer, no test), and the fix mirrors the evaluator's own `(count/total)*100` (`src/evaluators/blood_pressure.py:501`). CPU-unit-verified (bounded [0,100], monotonic in threshold, empty-tensor-safe) + ruff-clean. ⚠️ **Only future runs are corrected** — already-logged cloud runs (e.g. `swan-1`) keep their >100 values (SwanLab data is immutable).
- `src/utils/checkpoint_manager.py` — the **best-model slot** (`epoch=None`) now produces a `_best` filename suffix instead of an empty one (`_build_filename` + the `find_checkpoint` glob branch both changed `"" → "_best"`). So the rolling best checkpoint is `<direction>checkpoint_S_<seed>_best.pt` (periodic stays `_epoch_N`). Save, resume-load (`load_checkpoint_from_epoch=None`), and eval (`checkpoint_epoch=None`) all route through the same `build_path`, so they stay symmetric. ⚠️ **Convention change**: checkpoints saved by the *old* code (`..._S_<seed>.pt`, no suffix) will **not** be found by the new load/eval path — there are no checkpoints shipped in this snapshot, and `pretrain.sh`/`finetune.sh` re-save under the new name, so this only matters if you kept an old run's weights. The `pretrain.sh` discovery glob was tightened to `*checkpoint_S_*_best.pt` and both scripts pass `trainer.save_checkpoint_frequency=null` to disable periodic saving (keep only the best file). Unit-verified (`_best`/`_epoch_5` naming) + ruff-clean.
